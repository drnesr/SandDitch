...
def get_section_grid(source_df, axis_of_section='y', grid_value=1., default_value=20., output_method='3D'):
    ''' 
...
grid       = 0.25 # cm
levels     = get_legend_range(0.05, 0.45)
title_print='| Axi | sub | soil | elmt | emtr | CS  | TS |PosVol| NegVol | TotVol | PosAvg | NegAvg  |  TotAvg |      3D file name      |'
print (title_print)
print('-'*len(title_print))
out_print = '|{:^5}|{:^5}|{:^6}|{:^6}|{:^6}|{:5.1f}|{:4d}|{:6.2f}|{:8.2f}|{:+08.2f}|{:8.6f}|{:8.6f}|{:+8.6f}|{:^24}|'
out_frame = []
...
for i, data_frame_3d in enumerate(df3d): #[:1]):######
    name3d= file_names[dict_3d[i]]
    # Drawing contours of selected timesteps 
    for crosses in sections[dict_3d[i]]: #[:1]:######
        for time_step in time_steps[dict_3d[i]][1:]:
            # Plotting and calculating the 3D file
            plot_title = "Cross sectional contour of {} after time value={}.             CS@ {} direction, crossing at {}={} cm".                        format({0:'Theta', 1:'Head'}[variable], time_step,
                              {'x': 'Y-Z', 'y':'X-Z'}[section], section, crosses)
