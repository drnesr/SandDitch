...
#     print('is2d=', is2d)
    ...
    if mx - mn < 0.000000001:
        ...
        if output_the_contour:
            draw_contour(X, Z, M, levels, plot_title, x_step, z_step, 
                         mirror_x, mirror_z)
        else:
            return X, Z, M, levels
    if return_arrays:
...
#             prc_vol[-2] = prc_vol[-1]-prc_vol[-3]
    ...
    if calculate_volume:
        Lx, Lz = _x[0], _z[:,0]
        return integrate_volume(Lx, Lz, difference_matrix, 
                                get_average=calculate_average, 
                                separate_negatives=separate_negatives)
...
for i, data_frame in enumerate(df2d + df3d):
    ...
    if 'y' in dims[i]:
        sections[i]=section_locations * dims[i]['y'][1]
...
source_folder= 'C:/Users/DrNesr/Documents/Current'
