...
tab_df = get_results_for_grid(cont_y[0], cont_y2d[0])
tab_df.head()#+tab_df.tail()
pd.concat([tab_df.head(),tab_df.tail()])
...
tab_df = get_results_for_grid(cont_y[0], cont_y2d[0])
x_g, z_g = tab_df.x_cord.max()+1, tab_df.z_cord.max()+1
# x_g, z_g
x_vals, z_vals = sorted(list(set(tab_df.x_average))), sorted(list(set(tab_df.z_average)))
m_vals = list(tab_df.m_average)
# x_vals, z_vals , m_vals
x, z = np.meshgrid(x_vals, z_vals)
m_moisture=np.array(m_vals).reshape(x_g, z_g)
m_vol_all=np.array(list(tab_df.dif_vol_all)).reshape(x_g, z_g)
m_avg_all=np.array(list(tab_df.dif_avg_all)).reshape(x_g, z_g)
m_vol_ratio=np.array(list(tab_df['full_vol_ratio%'])).reshape(x_g, z_g)

plt.figure(1, figsize=(15,20))
num_rows, num_cols = 3, 4
idx=0

interpol = {0: 'none', 1: 'nearest', 2: 'bilinear', 3: 'bicubic',
             4: 'spline16', 5: 'spline36', 6: 'hanning', 7: 'hamming',
             8: 'hermite', 9: 'kaiser', 10: 'quadric', 11: 'catrom',
             12: 'gaussian', 13: 'bessel', 14: 'mitchell', 15: 'sinc',
             16: 'lanczos'}

for prop_var, prop_title in zip([m_moisture, m_vol_all, m_avg_all, m_vol_ratio], 
                      ['m_moisture', 'm_vol_all', 'm_avg_all', 'm_vol_ratio']):
    
    idx += 1

    plt.subplot(num_rows, num_cols, idx + 2* num_cols)
    cs = plt.contourf(x, z, prop_var)
    plt.contour(cs, colors='w')
    plt.grid(c='k', ls='-', alpha=0.3)
    plt.title(prop_title)

    plt.subplot(num_rows, num_cols, idx + num_cols)
    plt.imshow(np.flipud(prop_var), extent=(x.min(), x.max(), z.min(), z.max()),
               interpolation='none', cmap=cm.YlGn)
    plt.title(prop_title)

    plt.subplot(num_rows, num_cols, idx )
    plt.imshow(np.flipud(prop_var), extent=(x.min(), x.max(), z.min(), z.max()),
               interpolation=interpol[5], cmap=cm.YlGn)
    plt.title(prop_title)
...
# tests read_hydrus_data
read_hydrus_data(save_to_csv=False).head()    
...
# Test3 distance3d
# distance3d((1,2,3))
# distance3d((1,2,3), (4,1,7))


# In[25]:


# tests get_section_grid
# get_section_grid(theta, 'y')
# len(get_section_grid(theta, 'y')[0]), len(get_section_grid(theta, 'y')[0][0])


# In[26]:


# tests get_section_dataframes
# get_section_dataframes(theta)[0].shape, get_section_dataframes(theta)[1].shape
# output>>> ((66, 12), (431, 12))
# get_section_dataframes(theta, output='full').shape
# output>>> (497, 12)
# get_section_dataframes(theta, cross_at=20., tolerance=15., output='full').shape
...
data_frame= read_hydrus_data(save_to_csv=False)
v=0
...
print('x_vals{}, z_vals{}, X{}, Z{}, M{}'.format(x_vals.shape, 
                                                 ...
                                                 Z.shape, M.shape))
# src[['{}_T={}'.format('Th', time_step)]].head()
# src[['{}_T={}'.format('H', time_step)]].head()
# src[['{}_T={}'.format('V', time_step)]].head()


# In[28]:


# tests get_available_timesteps
data_frame = read_hydrus_data(save_to_csv=False)
...
# tests round_to_significance
for kjkj in [0.11,0.43, 0.25, 0.37,.07]:
    print(kjkj, 
          round_to_significance(kjkj,0.03), 
          round_to_significance(kjkj,0.03,'dn'))


# In[31]:


#tests get_legend_range
# get_legend_range(0.110961, 0.43000000000000005)
# mn,mx,rn= M.min(), M.max(), M.max()-M.min()
# mn,mx,rn
...
# Tests of reduce_crossed_at_list
print('Even list test')
cs_list= np.arange(-50, 0, 5)
print(cs_list)
print(len(cs_list), len(cs_list) %2, cs_list[0], cs_list[-1],
      cs_list[1:-1], cs_list[1:-1][::2])
print(reduce_crossed_at_list(cs_list))
print('\nOdd list test')
cs_list= np.arange(-50, 0, 6)
print(cs_list)
print(len(cs_list), len(cs_list) %2, cs_list[0], cs_list[-1],
      cs_list[1:-1], cs_list[1:-1][::2])
print(reduce_crossed_at_list(cs_list))
...
#=========================================================================    
# test of integrate_volume
sZ, sX=3,5
M=[]
for _ in range(sZ):
    M.append(np.random.randint(1,15,sX))
M=np.array(M)
x= np.linspace(0, 70, sX)
z= np.linspace(0, 120, sZ)

# integrate_volume(x, z, M)

# integrate_volume(x, z, M, method='Simpson'), calculate_volume(
#                  x, z, M, method='Trapezoidal'), calculate_volume(
#                  x, z, M, method='mean')

integrate_volume(x, z, M, get_average=True, separate_negatives=False), integrate_volume(x, z, M, get_average=True, separate_negatives=True)


# In[42]:


# draw_difference tests
# draw_difference(cont[1], cont[2], scale_from=1), draw_difference(
# cont[1], cont[2], scale_from=0)
# draw_difference(cont[1], cont[2], scale_from=1,
#                 calculate_volume=True, calculate_average=True), draw_difference(
#                 cont[1], cont[2], scale_from=0, 
#     calculate_volume=True, calculate_average=True)
# draw_difference(cont[1], cont[2], scale_from=1,
#                 calculate_volume=True, calculate_average=True, no_contours=True)
# draw_difference(cont[1], cont[2], scale_from=1,
#                 calculate_volume=False, calculate_average=True, no_contours=True)
...
# To draw all contour maps for specefic times and cross sections
data_frame = get_df_from_csv(output, 'pXZ_3D_sub_sand_sfr_1m.CSV')
print(get_full_dimensions(data_frame))
print(get_available_timesteps(data_frame))
...
i = -1
_ti="Cross sectional contour of {} after time value={}. CS@ {} direction, crossing at {}={} cm"
for crosses in [35, 45, 70]:
    for time_step in [180]:#[60, 120, 180, 360]:
        plot_title = _ti.format({0:'Theta', 1:'Head'}[variable], time_step,
                          {'x': 'Y-Z', 'y':'X-Z'}[section], section, crosses)
        i += 1
        # cont will hold a tuple of (X, Y, Z)
        cont_x.append(draw_full_contour(data_frame,variable, time_step, 
                                        grid, crosses, tol, section, 
                                        levels, plot_title, 
                                        x_step=15, z_step=25, 
                                        mirror_x=True, mirror_z=False))
...
# # Testing the function draw_difference
# cont_y[0], cont_y2d[0]
# for c_vp in [True, False]:
#     for c_a in [True, False]:
#         for s_n in [True, False]:
#             print ('\ncalculate_average={}, separate_negatives={}, \
# calculate_volume_percent={}'.format(c_a, s_n, c_vp))
#             print(draw_difference(cont_y[0], cont_y2d[0], calculate_volume=True, 
#                             scale_from=2,custom_levels=get_legend_range(0, 0.2),
#                             calculate_average=c_a, no_contours=True, 
#                             separate_negatives=s_n, 
#                             calculate_volume_percent=c_vp))


# OUT>>>
# calculate_average=True, separate_negatives=True, calculate_volume_percent=True
# ((0.2570336189624362, 2.937527073856414e-05, 8750.0), 
# (-242.0382469103468, -0.027661513932611063, 8750.0), 
# (-241.78121329138438, -0.0276321386618725, 8750.0), 
# 0.0001398757454138966, -inf, -0.13157550196245413)

# calculate_average=True, separate_negatives=False, calculate_volume_percent=True
# (-241.78121329138435, -0.0276321386618725, 8750.0, -0.13157550196245413)

# calculate_average=False, separate_negatives=True, calculate_volume_percent=True
# (0.2570336189624362, -242.0382469103468, -241.78121329138438, 
# 0.0001398757454138966, -inf, -0.13157550196245413)

# calculate_average=False, separate_negatives=False, calculate_volume_percent=True
# (-241.78121329138435, -0.13157550196245413)

# calculate_average=True, separate_negatives=True, calculate_volume_percent=False
# ((0.2570336189624362, 2.937527073856414e-05, 8750.0), 
# (-242.0382469103468, -0.027661513932611063, 8750.0), 
# (-241.78121329138438, -0.0276321386618725, 8750.0))

# calculate_average=True, separate_negatives=False, calculate_volume_percent=False
# (-241.78121329138435, -0.0276321386618725, 8750.0)

# calculate_average=False, separate_negatives=True, calculate_volume_percent=False
# (0.2570336189624362, -242.0382469103468, -241.78121329138438)

# calculate_average=False, separate_negatives=False, calculate_volume_percent=False
# -241.78121329138435
...
levels = get_legend_range(0.05, 0.45)
_ti = "Cross sectional contour of {} after time value={}. CS@ {} direction, crossing at {}={} cm"
for time_step in get_available_timesteps(data_frame)[0:]:
    plot_title = _ti.format({0:'Theta', 1:'Head'}[variable], time_step,
                      {'x': 'Y-Z', 'y':'X-Z'}[section], section, crosses)
    draw_full_contour(data_frame,variable, time_step, grid, crosses, tol, section, 
                      levels, plot_title)
...
for crosses in [25, 35, 45]:
    for time_step in [180]:#[60, 120, 180, 360]:
        plot_title = _ti.format({0:'Theta', 1:'Head'}[variable], time_step,
                          {'x': 'Y-Z', 'y':'X-Z'}[section], section, crosses)
        i += 1
        # cont will hold a tuple of (X, Y, Z)
        cont.append(draw_full_contour(data_frame,variable, time_step, 
                                      grid, crosses, tol, section, 
                                      levels, plot_title))
# double click the output pane to show the results
...
# draw_cross_sections(_m, _x, _z, direction='x', 
#                     crossed_at_list=None, number_of_sections=10)
...
# draw_cross_sections(_m, _x, _z, direction='z', 
#                     crossed_at_list=None, number_of_sections=10)
...
subfolders= retrieve_all_csv_files(source, get='all', retrieve_folders_only=True, 
                                   get_only_new=True, output_folder=output)
list(subfolders.values())


# In[35]:


retrieve_all_csv_files(source)#(get='2d')
...
Tp, Tn, Tt =  draw_difference(cont_y[0], cont_y2d[0],scale_from=0, 
                              custom_levels=get_legend_range(-.1, 0.2),
                               mirror_x=True, calculate_volume=True, 
                               calculate_average=True,
                               no_contours=True, separate_negatives=True )
print('For Positives, the diffr. vol. = {:0.3f}, & the avg. moisture = {:0.4f}'.
      format(Tp[0], Tp[1]))
print('For Negatives, the diffr. vol. = {:0.3f}, & the avg. moisture = {:0.4f}'.
      format(Tn[0], Tn[1]))
print('For Total    , the diffr. vol. = {:0.3f}, & the avg. moisture = {:0.4f}'.
      format(Tt[0], Tt[1]))
# print('The differential volume = {:0.3f}, and the average moisture = {:0.4f}'.
# format(v, i))
...
src='C:/Users/DrNesr/Documents/Current'
dst= source #'C:\zTest\Current2'
copy_required_files_and_folders(src, dst) 

retrieve_all_csv_files(source, get='all', retrieve_folders_only=False, 
                                   get_only_new=True, output_folder=output)
...
for i, data_frame in enumerate(df2d + df3d):
    print('For the case study         : {}'.format((the_2d_files+the_3d_files)[i]))
    ...
    if 'y' in dims[i]:
        sections[i]=section_locations * (dims[i]['y'][1]-dims[i]['y'][0])
...
# # ===========================================================
# # ===== Third, performing calculations on the dataframes ====
# # ===========================================================
...
# grid       = 0.25 # cm
# levels     = get_legend_range(0.05, 0.45)
# title_print='| Axi | sub | soil | elmt | emtr | CS  | TS |PosVol| NegVol \
# | TotVol | PosAvg | NegAvg  |  TotAvg |      3D file name      |'
# print (title_print)
# print('-'*len(title_print))
# out_print = '|{:^5}|{:^5}|{:^6}|{:^6}|{:^6}|{:5.1f}|{:4d}|{:6.2f}|{:8.2f}\
# |{:+08.2f}|{:8.6f}|{:8.6f}|{:+8.6f}|{:^24}|'
# out_frame = []
# # Looping through all 3D files and 2D files
# for i, data_frame_3d in enumerate(df3d):
#     name3d= file_names[dict_3d[i]]
#     # Drawing contours of selected timesteps 
#     for crosses in sections[dict_3d[i]]:
#         for time_step in time_steps[dict_3d[i]][1:]:
#             # Plotting and calculating the 3D file
#             plot_title = "Cross sectional contour of {} after time value={}. \
#             CS@ {} direction, crossing at {}={} cm".\
#                         format({0:'Theta', 1:'Head'}[variable], time_step,
#                               {'x': 'Y-Z', 'y':'X-Z'}[section], section, crosses)
...
#                                              is2d=True, 
#                                              output_the_contour=False)
#                 Tp, Tn, Tt =  draw_difference(cont_y3D, cont_y2d,
#                                               scale_from=0, 
...
#                                               no_contours=True, 
#                                               separate_negatives=True )
#                 part5 = name_parts_2d[5] if len(name_parts_2d[5])<=3 else name_parts_2d[5][1:4]
#                 outs = (name_parts_2d[0], name_parts_2d[2], name_parts_2d[3],
#                                        name_parts_2d[4], part5, crosses, time_step,
#                                         Tp[0], Tn[0], Tt[0], 
#                                         Tp[1],Tn[1], Tt[1])
#                 out_frame.append(outs)
#                 print(out_print.format(name_parts_2d[0], name_parts_2d[2], name_parts_2d[3],
#                                        name_parts_2d[4], part5,
#                                        crosses, time_step,
#                                        Tp[0], Tn[0], Tt[0], 
#                                        Tp[1],Tn[1], Tt[1], name3d[:-4]))
#                 pass
#             pass
#         pass
#     pass
...
# out_data_frame.to_csv(os.path.join(output, 'AnalysisResultsGross.CSV'))
# out_data_frame                
...
grid       = 0.25 # cm
levels     = get_legend_range(0.05, 0.45)
title_print='| n |Axi|sub|soil|elm|emtr|CS | TS |PosVol| NegVol | TotVol | PosAvg | NegAvg  |  TotAvg |PsV_Pr|NgV_Pr|TtV_Pr|  3D_filename  |'
# print (title_print)
# print('-'*len(title_print))
# out_print = '|{:3d}|{:^3}|{:^3}|{:^4}|{:^3}|{:^4}|{:3d}|{:4d}|{:6.2f}|{:8.2f}\
# |{:+08.2f}|{:.2E}|{:.2E}|{:.2E}|{:5.2f}|{:05.2f}|{:+05.2f}|{:^16}|'
out_frame = []
        ...
        for time_step in time_steps[dict_3d[i]][1:]: ########## [1:2]
            ...
            for j, data_frame_2d in enumerate(df2d): #[:1]):#######
                ...
                cont_y2d = draw_full_contour(data_frame_2d,variable, 
                                             ...
                                             output_the_contour=False)
#                 print(name3d, name2d)
#                 print('cont_y3D:',[cont_y3D[lk].shape for lk in range(4)], 
#                       '\ncont_y2D:',[cont_y2d[lk].shape for lk in range(4)])
...
#                 print(out_print.format(case_number, name_parts_2d[0], 
#                                     name_parts_2d[2], name_parts_2d[3],
#                                        name_parts_2d[4], part5,
#                                        int(crosses), time_step,
#                                        Tp[0], Tn[0], Tt[0], 
#                                        Tp[1],Tn[1], Tt[1], 
#                                        Vp*100., Vn*100., Vt*100., 
#                                        name3d[7:-4].replace('_','')))
...
out_data_frame=pd.DataFrame(out_frame, columns=title_print.replace('|',' ').split())
out_data_frame.to_csv(os.path.join(output, 'AnalysisResultsGrossFull.CSV'))

grid_res_df.to_csv(os.path.join(output, 'AnalysisResultsGrossGridded.CSV'))
...
out_data_frame.describe()


# In[367]:


# out_data_frame.groupby(['3D_name', '2D_name']).mean()
# center_df = out_data_frame[
#     (out_data_frame['CS']==66) | (out_data_frame['CS']==50)]
# at60pr_df = out_data_frame[
#     (out_data_frame['CS']==132*.6) | (out_data_frame['CS']==60)]
# at75pr_df = out_data_frame[
#     (out_data_frame['CS']==132*.75) | (out_data_frame['CS']==75)]
# at90pr_df = out_data_frame[
#     (out_data_frame['CS']==132*.9) | (out_data_frame['CS']==90)]
# center_df.groupby(['3D_name', '2D_name'])['TotAvg', 'TotVol'].mean()
# #.plot(kind='bar')
...
# retrieve_all_csv_files(HYDRUS_TXT, 
#                        get='pXZ_3D_srf_Silt_sht_rO1f', 
#                        retrieve_folders_only=False, 
#                        get_only_new=False, 
#                        output_folder=HYDRUS_CSV)


# In_Folder: H3D2_pXZ_3D_srf_Silt_sht_rO1f, Out_File: pXZ_3D_srf_Silt_sht_rO1f.CSV
# 
# ======= Converting HYDRUS files to CSV format ======
# 
# H3D2_pXZ_3D_srf_Silt_sht_rO1f... saved to pXZ_3D_srf_Silt_sht_rO1f.CSV
# 
# ====  ALL THE FILES WERE CONVERTED SUCCESSFULLY  ===
# 
...
plt.ioff()
i=0
data_frame_3d=df3d[i]
name3d= file_names[dict_3d[i]]
name_parts_3d=name3d[:-4].replace("_", " ").split() 
cross_sections = sections[dict_3d[i]]
time_stepsss=time_steps[dict_3d[i]][1:]
if name_parts_3d[0]=='Axi':
    axisym=True
else:
    axisym=False
crosses=cross_sections[0]
tt=2
time_step=time_stepsss[tt]

plot_title = "Cross sectional contour of {} after time value={}. CS@ {} direction, crossing at {}={} cm".                        format({0:'Theta', 1:'Head'}[variable], time_step,
                              {'x': 'Y-Z', 'y':'X-Z'}[section], section, crosses)
    
name3d, cross_sections, time_stepsss, plot_title
...
fig3D_name= '{}_t{}c{}'.format(name3D_mod,tt,crosses)
...
data_frame_2d=df2d[i]


# In[50]:


cont_y2d_full = draw_full_contour(data_frame_2d,variable, time_step, 
                                 grid, crosses, tol, section,levels, 
                                 plot_title, x_step=15, z_step=25, 
                                 mirror_x=True, mirror_z=False, 
                                 is2d=True, output_the_contour=False,
...
name_parts_2d=name2d[:-4].replace("_", " ").split()
np2d=name_parts_2d
name2D_mod = '_'.join([np2d[3], np2d[1], np2d[0], np2d[2], np2d[4], np2d[5]])
...
fig_dif =  draw_difference(cont_y3D, cont_y2d, scale_from=0, 
                                  custom_levels=None,
                                  mirror_x=True, calculate_volume=False, 
...
fig_dif_name = 'dif_{}_{}'.format(name3D_mod, name2d[16:-4])
fig_dif_name
...
for level in fig_dif.collections:
...
out_data_frame=pd.DataFrame(out_frame, columns=title_print.replace('|',' ').split())
out_data_frame.to_csv(os.path.join(output, 'AnalysisResultsGrossFull_New_2Lph.CSV'))

grid_res_df.to_csv(os.path.join(output, 'AnalysisResultsGrossGridded_New_2Lph.CSV'))
grid_res_df.head(), out_data_frame.head()
